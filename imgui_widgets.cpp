...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags, float scale)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    RenderArrow(window->DrawList, bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f), ImMax(0.0f, (size.y - g.FontSize) * 0.5f)), text_col, dir, scale);
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    else
    {
        ...
        const ImVec2 buttonsPos = GetCursorPos();
        SetCursorPosY(GetCursorPosY() + button_size * .5f);
        if (ArrowButtonEx("-", ImGuiDir_Down, ImVec2(button_size, button_size * .5f), button_flags, .5F))
        {
            DataTypeApplyOp(data_type, '-', p_data, p_data, g.IO.KeyCtrl && p_step_fast ? p_step_fast : p_step);
            value_changed = true;
        }
        SetCursorPos(buttonsPos);
        if (ArrowButtonEx("+", ImGuiDir_Up, ImVec2(button_size, button_size * .5f), button_flags, .5F))
        {
            DataTypeApplyOp(data_type, '+', p_data, p_data, g.IO.KeyCtrl && p_step_fast ? p_step_fast : p_step);
            value_changed = true;
        }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueCircle)
    {
        // Hue wheel + SV triangle logic
        InvisibleButton("hsv", ImVec2(sv_picker_size + style.ItemInnerSpacing.x + bars_width, sv_picker_size));
        if (IsItemActive() && !is_readonly)
...
//ExtremeEngine changes
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled, ImTextureID user_texture_id)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    else
    {
        ...
        if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible)
        {
            float finalOffsetLabel = offsets->OffsetLabel;
            if (user_texture_id != NULL)
            {
                window->DrawList->AddImage(user_texture_id, pos, pos + ImVec2(label_size.y, label_size.y));
                finalOffsetLabel += label_size.y + offsets->Spacing;
            }
            RenderText(pos + ImVec2(finalOffsetLabel, 0.0f), label);
            if (icon_w > 0.0f)
                RenderText(pos + ImVec2(offsets->OffsetIcon, 0.0f), icon);
            if (shortcut_w > 0.0f)
            {
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    const ImVec2 rectMin = ImVec2(bb.Max.x, bb.Min.y + 5);

    draw_list->AddRectFilled(rectMin, bb.Min, col);
    //draw_list->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_Border));
